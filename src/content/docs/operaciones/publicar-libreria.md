...
:::note[El historial completo vive en otra página]
La referencia canónica, con el detalle de cada versión y qué implica para los
consumidores, es [Versiones de @hagemsa/auth-guard](/versiones/auth-guard/). Esta
tabla es un resumen para no tener que salir de acá al publicar.
:::

| Versión | Fecha | Cambios |
|---|---|---|
| **0.6.0** | 2026-09-24 | **Claim `socioCuentas`.** Las cuentas y contratos que el socio de negocio de la cuenta tiene asignados en BC-01 viajan en el token, como `{ codigo, nombre, tipo }[]`. Sirve para filtrar por cuenta sin llamar a BC-01. Cambio **aditivo**: campo opcional en `AuthContext` y `JwtPayload`, más el tipo `SocioCuentaJwt` exportado desde el índice. Un consumidor en 0.5.0 o 0.4.0 no se rompe. El claim **no viaja** si la cuenta no tiene socio, ni si el socio no tiene cuentas: nunca llega como lista vacía. Es una **foto** del momento en que se vinculó el socio, no el estado vivo de BC-01. |
| **0.5.0** | 2026-07-30 | **Endurecimiento de seguridad**, sin cambios de API. Tres correcciones: (1) amplificación de peticiones contra el Auth Service — el `kid` se lee del token sin verificar la firma, así que un anónimo lo elegía y cada cache miss forzaba un refresco del JWKS; ahora hay negative-cache de kids desconocidos y cooldown entre refrescos. (2) Un fallo del JWKS devolvía **500**; ahora devuelve 401, que es lo correcto: es un token que no se pudo verificar, no un fallo del backend. (3) El cache de blacklist crecía sin límite; queda acotado a 10 000 entradas. **No hay que tocar código ni variables**: se actualiza la dependencia y se redespliega. |
